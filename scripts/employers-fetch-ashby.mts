...
import { refreshAshbyEmployerSources } from '../src/shared/services/ashby-refresh.ts';
...
try {
  ...
  const timer = new Promise<never>((_, reject) => setTimeout(() => reject(new Error('ASHBY_COMMAND_TIMEOUT')), Math.max(1_000, timeoutMs)));
  const summary = await Promise.race([refreshAshbyEmployerSources({ ...(source ? { sourceIds: [source] } : {}), ...(company ? { companyRecordId: company } : {}), all: process.argv.includes('--all'), concurrency }), timer]);
