...
import { refreshLeverEmployerSources } from '../src/shared/services/lever-refresh.ts';
...
try {
  ...
  const timer = new Promise<never>((_, reject) => setTimeout(() => reject(new Error('LEVER_COMMAND_TIMEOUT')), Math.max(1_000, timeoutMs)));
  const summary = await Promise.race([refreshLeverEmployerSources({ ...(source ? { sourceIds: [source] } : {}), ...(company ? { companyRecordId: company } : {}), all: process.argv.includes('--all'), concurrency }), timer]);
