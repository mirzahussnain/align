...
async function snapshot(input: { id: string; title: string; employer: string; availability: 'FULL' | 'PARTIAL' | 'EXTERNAL_ONLY'; providerDescription?: string; userDescription?: string; references: Array<{ provider: 'ADZUNA' | 'REED' | 'JOOBLE'; id: string; url: string }> }) {
  ...
  return prisma.jobSnapshot.upsert({
    ...
    create: { canonicalJobId: input.id, title: input.title, normalisedTitle: input.title.toLowerCase(), employerName: input.employer, normalisedEmployerName: input.employer.toLowerCase(), importedByUserId: input.userDescription ? userId : null, locationText: 'Leeds, UK', country: 'UK', workStyle: 'HYBRID', providerDescription: input.providerDescription, userSuppliedDescription: input.userDescription, descriptionAvailability: input.availability, selectedDescriptionSource: selected?.source, selectedDescriptionHash: selected ? hash(selected.text) : null, dedupeFingerprint: input.id, vacancySponsorshipSignal: { fixture: true, note: 'Development fixture; not sponsor-register evidence.' }, fetchedAt: now, firstSeenAt: now, lastSeenAt: now, providerReferences: { create: input.references.map((reference) => ({ provider: reference.provider, providerJobId: reference.id, providerUrl: reference.url, firstSeenAt: now, lastSeenAt: now })) } },
    update: { title: input.title, importedByUserId: input.userDescription ? userId : null, providerDescription: input.providerDescription, userSuppliedDescription: input.userDescription, descriptionAvailability: input.availability, selectedDescriptionSource: selected?.source, selectedDescriptionHash: selected ? hash(selected.text) : null, lastSeenAt: now, fetchedAt: now },
  });
}
...
await prisma.jobMatchRequest.create({ data: { userId, profileId, jobSnapshotId: partial.id, selectedDescriptionSource: 'PROVIDER_PARTIAL', selectedDescriptionHash: hash(partial.providerDescription!), descriptionText: partial.providerDescription!, partialDescriptionAccepted: true, status: 'PREPARED', expiresAt: new Date(Date.now() + 30 * 60_000) } });
