...
describe("provider counts describe the jobs actually shown", () => {
  it("excludes results that filtering removed from the rendered list", async () => {
    ...
    searchProvidersInteractive.mockResolvedValue(
      fanOut([
        ...
        providerResult("ADZUNA", [
          job({ provider: "ADZUNA", remoteType: "REMOTE" }),
        ]),
      ]),
    );

    const { body } = await call("query=countcheck&remoteType=REMOTE");
    const counts = body.meta.providerCounts as {
      provider: string;
    ...
    const contributed = counts.reduce(
      (total, count) => total + count.uniqueContributed,
      0,
    );
  ...
  it("credits no contribution to a provider whose results were all filtered out", async () => {
    ...
    expect(
      counts.find((count) => count.provider === "REED")!.uniqueContributed,
    ).toBe(0);
  });
});
...
describe("response cache is keyed by the page and the order it holds", () => {
  it("does not let Load more overwrite the first page", async () => {
    ...
    searchProvidersInteractive.mockResolvedValueOnce(
      fanOut([providerResult("REED", pageOne)]),
    );
    ...
    searchProvidersInteractive.mockResolvedValueOnce(
      fanOut([providerResult("REED", pageTwo)]),
    );
    const more = await call(
      `query=cachecheck&location=Leeds&sessionId=${first.body.sessionId}`,
    );
  ...
  it("does not serve a descending page to an ascending request", async () => {
    ...
    searchProvidersInteractive.mockResolvedValue(
      fanOut([providerResult("REED", jobs)]),
    );

    const descending = await call(
      "query=sortcheck&location=Hull&sortBy=salary_desc",
    );
    expect(descending.body.jobs[0].title).toBe("Higher paid");

    const ascending = await call(
      "query=sortcheck&location=Hull&sortBy=salary_asc",
    );
...
describe("load more", () => {
  it("does not repeat jobs the session has already shown", async () => {
    ...
    searchProvidersInteractive.mockResolvedValueOnce(
      fanOut([providerResult("REED", [shared])]),
    );
    ...
    searchProvidersInteractive.mockResolvedValueOnce(
      fanOut([
        providerResult("REED", [
          shared,
          job({ provider: "REED", title: "Genuinely new" }),
        ]),
      ]),
    );
    const more = await call(
      `query=repeatcheck&location=Derby&sessionId=${first.body.sessionId}`,
    );

    expect(more.body.jobs.map((entry: NormalisedJob) => entry.title)).toEqual([
      "Genuinely new",
    ]);
  });
});
