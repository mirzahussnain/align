...
export async function POST(request: NextRequest, context: { params: Promise<{ jobSnapshotId: string }> }) { return withErrorHandler(async () => {
  ...
  const intelligence = await assessAndPersistJobIntelligence({ jobSnapshotId, careerTrack: track ? { targetRoleTitle: track.targetRoleTitle, occupationFamily: track.targetOccupation, industry: track.targetIndustry, seniority: track.targetSeniority } : null, candidateFacts });
