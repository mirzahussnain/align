...
export const ManualEligibilityStep = forwardRef<ProfileStepHandle, { initial: { country: string; city: string; visaStatus: string; visaExpiry: string } }>(function ManualEligibilityStep({ initial }, ref) {
  ...
  return <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
    ...
    <div className={needsExpiry ? '' : 'sm:col-span-2'}>
      ...
      <div className="relative"><select id="eligibility-visa" value={form.visaStatus} onChange={(event) => set('visaStatus', event.target.value)} aria-describedby="eligibility-visa-hint" className="w-full appearance-none rounded-xl border border-neutral-300 bg-white px-4 py-2.5 pr-9 text-sm text-neutral-900 transition-colors focus:border-accent-purple focus:outline-none focus:ring-2 focus:ring-accent-purple/15"><option value="">Prefer not to say yet</option>{VISA_STATUSES.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select><ChevronDown className="pointer-events-none absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-neutral-400" aria-hidden="true" /></div>
