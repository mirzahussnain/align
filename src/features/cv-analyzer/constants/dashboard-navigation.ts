export interface NavigationItem {
  id: string;
  label: string;
  type: 'standard' | 'premium';
  /** The engine category this item maps to, when it's scored by the engine. */
  mappedCategoryId?: string;
}

export interface NavigationGroup {
  id: string;
  label: string;
  items: NavigationItem[];
}

export const DASHBOARD_NAVIGATION_GROUPS: readonly NavigationGroup[] = [
  {
    id: 'overview',
    label: 'OVERVIEW',
    items: [
      { id: 'overview', label: 'Live Resume Analysis', type: 'standard' }
    ]
  },
  {
    id: 'content',
    label: 'CONTENT',
    items: [
      { id: 'atsReadability', label: 'ATS Parse Rate', type: 'standard', mappedCategoryId: 'atsReadability' },
      { id: 'impactStatements', label: 'Quantifying Impact', type: 'standard', mappedCategoryId: 'impactStatements' },
      { id: 'repetition', label: 'Repetition', type: 'standard' },
      { id: 'professionalSummary', label: 'Spelling & Grammar', type: 'standard', mappedCategoryId: 'professionalSummary' },
      { id: 'keywordDensity', label: 'Role Keywords', type: 'standard', mappedCategoryId: 'evidenceCoverage' },
      { id: 'bulletsConsistency', label: 'Bullets Consistency', type: 'standard' }
    ]
  },
  {
    id: 'sections',
    label: 'SECTIONS',
    items: [
      { id: 'essentialSections', label: 'Essential Sections', type: 'standard', mappedCategoryId: 'sectionCompleteness' },
      { id: 'contactInfo', label: 'Contact Information', type: 'standard' },
      { id: 'sectionOrder', label: 'Sections Order', type: 'standard', mappedCategoryId: 'sectionCompleteness' }
    ]
  },
  {
    id: 'ats-essentials',
    label: 'ATS ESSENTIALS',
    items: [
      { id: 'fileFormatSize', label: 'File Format & Size', type: 'standard', mappedCategoryId: 'formatting' },
      { id: 'formatting', label: 'Design', type: 'standard', mappedCategoryId: 'formatting' },
      { id: 'emailAddress', label: 'Email Address', type: 'standard' },
      { id: 'headerLinks', label: 'Header Links', type: 'standard' },
      { id: 'fileName', label: 'File Name Check', type: 'standard' },
      { id: 'datesLinks', label: 'Dates & Links', type: 'standard' }
    ]
  },
  {
    id: 'hr-red-flags',
    label: 'HR RED FLAGS',
    items: [
      { id: 'credibility', label: 'Credibility', type: 'standard' },
      { id: 'interviewRisks', label: 'Interview Risks', type: 'standard' },
      { id: 'linkedinMatch', label: 'LinkedIn Match', type: 'standard' }
    ]
  },
  {
    id: 'discrimination',
    label: 'DISCRIMINATION',
    items: [
      { id: 'compliance', label: 'UK Compliance', type: 'standard', mappedCategoryId: 'compliance' },
      { id: 'credentials', label: 'Credentials & Licences', type: 'standard', mappedCategoryId: 'credentials' },
      { id: 'relevance', label: 'Role Relevance', type: 'standard' }
    ]
  },
  {
    id: 'seniority',
    label: 'SENIORITY',
    items: [
      { id: 'roleTarget', label: 'Role Target', type: 'standard' }
    ]
  }
] as const;

export const JOB_MATCH_NAVIGATION = [
  { id: 'summary', label: 'Score & Summary', iconName: 'Target' },
  { id: 'skills', label: 'Skill Alignment', iconName: 'Target' },
  { id: 'domain', label: 'Domain & Eligibility', iconName: 'Briefcase' },
  { id: 'scoring', label: 'Score Breakdown', iconName: 'BarChart2' },
  { id: 'rewrites', label: 'Tailored Rewrites', iconName: 'FileEdit' },
  { id: 'strategy', label: 'Rewrite Strategy', iconName: 'LayoutList' }
] as const;

