...
// "Training" is uncountable, "Other evidence" already plural — everything else pluralises.
function heading(kind: ManagedEvidenceKind) { const singular = CONFIG[kind].singular; return `${singular[0].toUpperCase()}${singular.slice(1)}${kind === 'other' || kind === 'training' ? '' : 's'}`; }
...
export default function StructuredEvidenceManager({ kind, initial, profileId }: { kind: ManagedEvidenceKind; initial: Row[]; profileId: string }) {
  ...
  const openCreate = () => { setEditor({ mode: 'create', evidenceType: kind, draftId: draftId(), draft: blank(kind) }); setFeedback(''); };
  const openEdit = (record: Row) => { if (!record.id) return; setEditor({ mode: 'edit', evidenceType: kind, recordId: record.id, draft: { ...record } }); setFeedback(''); };
  ...
  function save() {
    ...
    startTransition(async () => {
      ...
      closeEditor(); setFeedback(`${config.singular[0].toUpperCase()}${config.singular.slice(1)} saved.`);
    });
  }
  function remove(record: Row) {
    ...
    startTransition(async () => { const result = await deleteManagedEvidence(kind, record.id!, profileId); if (!result.ok) return setFeedback(result.error); setRecords((current) => current.filter((item) => item.id !== record.id)); setFeedback(result.mayHaveStaleApprovals ? 'Deleted. Previous evidence approvals were not remapped.' : 'Deleted.'); });
  }
  return <div><div className="flex flex-col items-start gap-2 sm:flex-row sm:items-center sm:justify-between sm:gap-4"><div><h2 className="text-lg font-bold text-neutral-900">{heading(kind)}</h2><p className="mt-1 text-xs text-neutral-500">These reusable records are available in the evidence selector when tailoring a CV.</p></div></div>
    ...
    {!activeEditor && <div className="mt-4"><button type="button" onClick={openCreate} className="inline-flex items-center gap-1.5 rounded-full border border-neutral-300 px-4 py-2 text-xs font-semibold text-neutral-700 hover:bg-neutral-100"><Plus className="h-3.5 w-3.5" /> Add {config.singular}</button></div>}
