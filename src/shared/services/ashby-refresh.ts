...
export async function refreshAshbyEmployerSources(input: { sourceIds?: string[]; companyRecordId?: string; all?: boolean; concurrency?: number } = {}): Promise<AshbyRefreshSummary> {
  ...
  async function worker() { while (cursor < sources.length) {
    ...
    try {
      ...
      summary.failed += 1; if (errorCode === 'ASHBY_TIMEOUT') summary.timedOut += 1; if (errorCode === 'ASHBY_MALFORMED_RESPONSE') summary.malformedPayloads += 1; summary.perSourceFailures[errorCode] = (summary.perSourceFailures[errorCode] ?? 0) + 1;
