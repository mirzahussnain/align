...
export async function materialiseSearchJobCards(
  ...
  return materialised.map(({ job, id }) => {
    ...
    // Server-computed, exactly as on the durable-snapshot card path. The
    // description badge must never be inferred in React from text length, and
    // must never be derived from `freshness`, which is snapshot age.
    ...
    return {
      ...
      sponsorEvidenceSummary: sponsorSummary(
        job.sponsorSignal.registerMatchStatus,
      ),
