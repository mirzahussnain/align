...
export type GreenhouseRefreshSummary = { attempted: number; successful: number; empty: number; failed: number; timedOut: number; jobsRetrieved: number; uniqueJobsPersisted: number; duplicateJobsMerged: number; invalidUrls: number; malformedPayloads: number; durationMs: number; perSourceFailures: Record<string, number>; sources: Array<{ sourceId: string; identifier: string; status: 'SUCCESS' | 'EMPTY' | 'FAILED'; jobs: number; errorCode?: string; durationMs: number }> };
...
export async function refreshGreenhouseEmployerSources(input: { sourceIds?: string[]; companyRecordId?: string; all?: boolean; concurrency?: number } = {}): Promise<GreenhouseRefreshSummary> {
  ...
  const summary: GreenhouseRefreshSummary = { attempted: sources.length, successful: 0, empty: 0, failed: 0, timedOut: 0, jobsRetrieved: 0, uniqueJobsPersisted: 0, duplicateJobsMerged: 0, invalidUrls: 0, malformedPayloads: 0, durationMs: 0, perSourceFailures: {}, sources: [] };
  ...
  async function worker() { while (cursor < sources.length) {
    ...
    try {
      ...
      summary.successful += 1; summary.jobsRetrieved += result.jobs.length; summary.invalidUrls += result.invalidUrls; summary.duplicateJobsMerged += result.duplicateProviderJobIds;
