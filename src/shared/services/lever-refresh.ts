...
const errorCodeOf = (error: unknown) => error instanceof Error ? error.message.slice(0, 120) : 'LEVER_UNAVAILABLE';

/** Explicit administrative refresh only. Interactive search never calls this. */
export async function refreshLeverEmployerSources(input: { sourceIds?: string[]; companyRecordId?: string; all?: boolean; concurrency?: number; limit?: number } = {}): Promise<LeverRefreshSummary> {
  ...
  async function worker() { while (cursor < sources.length) {
    ...
    try {
      ...
      summary.failed += 1; if (errorCode === 'LEVER_TIMEOUT') summary.timedOut += 1; if (errorCode === 'LEVER_MALFORMED_RESPONSE') summary.malformedPayloads += 1; summary.perSourceFailures[errorCode] = (summary.perSourceFailures[errorCode] ?? 0) + 1;
